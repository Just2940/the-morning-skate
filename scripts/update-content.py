...
def log_error(where, exc, extra=""):
    """Unified error logger for swallowed exceptions.

    Replaces `except: pass` / `except: return []` patterns. The function
    returns None so callers can `return log_error(...) or []` if they
    want to continue with an empty result but still surface the failure.
    """
    tb = traceback.format_exc(limit=2).strip().splitlines()
    tail = tb[-1] if tb else str(exc)
    LOG.warning(f"[{where}] {type(exc).__name__}: {exc} | {tail}" + (f" | {extra}" if extra else ""))
    return None

def safe_str(x, default=""):
    """str() that won't produce the literal word 'None'.

    Today's data.json had `"live_strip": None` serialize to JSON null, but
    any field where str(x) is called on a possibly-None value will leak the
    string 'None' into the UI. Use safe_str() wherever user-visible text
    is coerced from a non-string value.
    """
    if x is None:
        return default
    if isinstance(x, str):
...
def truncate_at_word(s, n, suffix=""):
    """Truncate a string to ≤n chars, always at a word boundary.

    Replaces ad-hoc `[:N]` slices that cut mid-word (ticker headlines,
    dek summaries). If truncation occurs, optionally append `suffix`
    (default empty — do NOT use '...' since Section 0.4 bans it).
    """
    if not isinstance(s, str) or len(s) <= n:
        return s or ""
    # Respect validator: ellipsis is banned, so suffix must not contain U+2026 or '...'
    if "\u2026" in suffix or "..." in suffix:
        suffix = ""
    cut = s[:n - len(suffix)] if suffix else s[:n]
    # Back up to last whitespace so we don't split a word
    sp = cut.rfind(" ")
    if sp > n * 0.6:  # only back up if reasonable
        cut = cut[:sp].rstrip()
    return cut + suffix

def format_score(a, b, sep="-"):
    """Score formatter that preserves tight hyphens: '5-2' not '5 - 2'.

    Use this INSTEAD of f'{a}-{b}' inside prose that later passes through
    sanitize_ascii. The global U+2013 replacement no longer inserts spaces
    (see BANNED_PUNCT_REPLACEMENTS), so this is belt-and-braces for
    cases where source text uses real en-dashes in scores.
    """
    a = safe_str(a, "").strip()
    b = safe_str(b, "").strip()
    if not a or not b:
...
BANNED_PUNCT_REPLACEMENTS = {
    "\u2014": " - ",  # em-dash (prose) — keep spaced per AP/Chicago style
    "\u2013": "-",    # en-dash (scores/ranges) — tight per AP/Chicago style
    ...
    "\u00B7": "-",    # middle dot (U+00B7) -> hyphen (for name separators)
}

# Contract self-check: at module load, assert we cover every char in the
# validator's BANNED_CHARS. Drift here is the #1 cause of recurring pipeline
# failures — the validator rejects something update-content.py didn't know
# to strip. Running this at startup catches regressions before build_data().
_VALIDATOR_BANNED = {
    "\u2014", "\u2013", "\u2026", "\u201C", "\u201D", "\u2018", "\u2019"
}
_missing = _VALIDATOR_BANNED - set(BANNED_PUNCT_REPLACEMENTS.keys())
if _missing:
    raise RuntimeError(
        f"BANNED_PUNCT_REPLACEMENTS drift vs validate_content.py: "
        f"missing replacements for {sorted(_missing)}. Add them or the "
        f"next build will fail Section 0.3."
    )
...
def sanitize_ascii(text):
    ...
    0.3 banned punctuation stripped: em-dash becomes ' - ', en-dash becomes
    '-' (tight — preserves scores like '5-2'), curly quotes become straight
    quotes, ellipsis becomes a space. HTML entities in upstream text
    (AI output, RSS) are decoded back to Unicode first so entities like
...
def fetch_espn_articles(team_key, limit=8):
    ...
    for item in data.get("articles", []):
        ...
        articles.append({
            ...
            "dek": truncate_at_word(item.get("description", ""), 200),
            "date": date_display,
            "link": link,
            "days_old": days_old,
            "type": item.get("type", ""),
                ...
                for item in data[:limit]:
                    ...
                    if slug and headline:
                        ...
                        articles.append({
                            ...
                            "source_class": "web",
                            "headline": headline,
                            "dek": truncate_at_word(summary, 200) if summary else "",
                            "date": "",
                            "link": link,
                            "days_old": 0,
                            "type": "news",
                ...
                for item in root.findall(".//item")[:limit]:
                    ...
                    if headline and link:
                        articles.append({
                            ...
                            "source_class": "web",
                            "headline": headline,
                            "dek": truncate_at_word(desc, 200) if desc else "",
                            "date": "",
                            "link": link,
                            "days_old": 0,
                            "type": "news",
...
def build_homepage_stories_from_articles(all_team_facts, all_team_articles):
    ...
    for team_key in ["leafs", "jays", "raptors", "commanders"]:
        ...
        # Priority scoring
        # NOTE: significance, not just recency. A morning briefing shouldn't
        # lead with "your team lost last night" unless there's nothing better.
        # Win recaps and playoff stakes outrank loss recaps.
        score = 0
        if "playoffs" in phase_id:
            score += 100  # Playoff teams always top priority
        last_result = (recent[0].get("result") if recent else "") or ""
        if recent:
            try:
        ...
        if "offseason" in phase_id or "draft" in phase_id or "ended" in phase_id:
            score += 5
        # Editorial weighting: wins boost, losses penalize (for FEATURED slot only
        # this matters; two_up / extra tolerate loss recaps).
        top_art = articles[0] if articles else {}
        top_is_recap = (top_art.get("type") or "").lower() == "recap"
        if top_is_recap:
            if last_result == "W":
                score += 15
            elif last_result == "L":
                score -= 20  # Pull loss recaps out of the featured slot
        # Non-recap editorial stories (signings, trades, injury reports) get
        # a small bump -- these age better than a previous-night score.
        if not top_is_recap and top_art.get("headline"):
            score += 8
    ...
    for candidate in team_candidates[:4]:
        ...
        # Ensure dek isn't too long -- Section 0.3 bans U+2026 ellipsis and
        # Section 0.4 bans literal "...", so truncate_at_word does a clean
        # word-boundary cut with no trailing punctuation.
        if len(dek) > 250:
            dek = truncate_at_word(dek, 250)
...
def build_key_numbers(team_key, team_info, standings, recent, phase_info=None):
    ...
    # audit caught "31 games back" in a Leafs LOTL where the number was
    # actually a points differential. Wrong label -> Perplexity picks up
    # "games back" and parrots it. Keeping this label accurate per-league
    # removes the source of that class of hallucination.
...
# ------------------------------------------------------------------------- #
# Hard-Fact Verifier — cross-checks LOTL prose against league APIs          #
# ------------------------------------------------------------------------- #
# The single biggest source of trust-breaking bugs has been Perplexity
# hallucinating hard facts (coach names, seeds, games-back numbers, player
# affiliations). Sanitizers can't catch editorial-grade errors; only
# cross-referencing authoritative league data can.
#
# This verifier runs AFTER Perplexity generation and BEFORE the LOTL ships.
# If a hard fact in the prose contradicts ESPN data, we reject the LOTL and
# fall back to a deterministic ESPN-based template.

def _fetch_team_coach(team_key):
    """Return the current head coach's full name per ESPN, or None on failure.

    Uses ESPN's sports.core API (which exposes a coaches endpoint) with a
    graceful fallback: if the call fails, we simply can't verify, so we
    return None and the caller treats the coach claim as unknown
    (not wrong).
    """
    cfg = TEAMS.get(team_key, {})
    sport = cfg.get("espn_sport")
    league = cfg.get("espn_league")
    tid = cfg.get("espn_team_id")
    if not (sport and league and tid):
        return None
    # Try current season first, then fall back to prior season if needed.
    import datetime as _dt
    year_guess = _dt.datetime.now().year
    for yr in (year_guess, year_guess - 1):
        url = f"https://sports.core.api.espn.com/v2/sports/{sport}/leagues/{league}/seasons/{yr}/teams/{tid}/coaches"
        data = espn_fetch(url)
        if not data:
            continue
        ...
        if not items:
            continue
        ref = items[0].get("$ref") if isinstance(items[0], dict) else None
        if not ref:
            # Some responses embed the coach directly.
            candidate = items[0] if isinstance(items[0], dict) else {}
            first = candidate.get("firstName") or ""
            last = candidate.get("lastName") or ""
            if first or last:
                return f"{first} {last}".strip()
            continue
        coach = espn_fetch(ref)
        if not coach:
            continue
...
def _extract_seed_from_standings(team_key, standings):
    """Pull the team's own seed/rank from the standings structure, if present.

    Falls back to None if standings is empty or unparseable. The caller treats
    None as 'can't verify', not 'contradicted'.
    """
    if not standings or not isinstance(standings, dict):
        return None
    panes = standings.get("panes") or []
    for pane in panes:
        for row in pane.get("rows") or []:
...
def verify_hard_facts(text, team_key, team_info, standings, coach_name=None):
    """Cross-check LOTL prose against authoritative league data.

    Returns (ok, problems). If ok=False the LOTL should be rejected and a
    fallback template used. If ok=True, the text is safe to ship (though
    `problems` may still contain warnings for logging).
    """
    if not text:
        return True, []
    problems = []
    critical = False  # True if any problem is severe enough to reject the LOTL

    text_lower = text.lower()

    # ---- 1. Coach-firing / coach-name claims ----
    # If the body claims a coach was fired/hired/let-go, the named coach must
    # match the actual head coach (or prior coach) per ESPN. Wrong name here
    # = the Keefe-fired-yesterday bug, which is unrecoverable.
    # Case-sensitive: name components must be capitalized so "was", "is", etc.
    # don't get swallowed into the name capture group.
    firing_re = re.compile(
        r"\b(?:[Cc]oach|[Hh]ead\s+[Cc]oach|HC)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,2})\b\s+"
        r"(?:was|is|got|just|has\s+been)\s+"
        r"(?:fired|let\s+go|dismissed|relieved|terminated|axed|out\b|gone\b)",
    )
    hiring_re = re.compile(
        r"\b(?:[Nn]ew(?:ly)?(?:[-\s]hired)?|[Jj]ust[-\s]hired|[Hh]ired)\s+"
        r"(?:head\s+)?coach\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,2})\b",
    )
    # Recency markers in the body that make stale personnel news fatal.
    recency_re = re.compile(
        r"\b(?:yesterday|today|this\s+week|last\s+week|overnight|this\s+morning|"
        r"hours?\s+ago|moments?\s+ago|just\s+now|breaking|recently|"
        r"in\s+a\s+shock(?:ing)?\s+move)\b",
        re.IGNORECASE,
    )
    has_recency = bool(recency_re.search(text))

    for pat, kind in ((firing_re, "firing"), (hiring_re, "hiring")):
        m = pat.search(text)
        if not m:
            continue
        claimed = m.group(1).strip()
        if coach_name:
            # Case A: claim is that CURRENT coach was fired — flatly wrong.
            if kind == "firing" and _names_match(claimed, coach_name):
                problems.append(
                    f"LOTL claims current coach {claimed} was fired, but ESPN shows {coach_name} is still head coach"
                )
                critical = True
            # Case B: claim that a NON-CURRENT coach was fired WITH a recency marker.
            # This is the Keefe-fired-yesterday bug: the named coach isn't current,
            # but the LOTL implies it's fresh news. Reject.
            if kind == "firing" and not _names_match(claimed, coach_name) and has_recency:
                problems.append(
                    f"LOTL claims {claimed} was fired recently, but ESPN shows {coach_name} is head coach — stale personnel claim"
                )
                critical = True
            # Case C: claim of a NEW hire that doesn't match ESPN's current coach.
            if kind == "hiring" and not _names_match(claimed, coach_name):
                problems.append(
                    f"LOTL claims new coach {claimed}, but ESPN shows {coach_name} is head coach"
                )
                critical = True

    # ---- 2. Standings seed verification ----
    # Body claims like "15th seed" or "3rd in Atlantic" must be within 2 of the
    # actual seed. We tolerate off-by-one because the standings API and the
    # LLM may lag by a day.
    actual_seed = _extract_seed_from_standings(team_key, standings)
    if actual_seed is not None:
        seed_claim_re = re.compile(r"\b(\d{1,2})(?:st|nd|rd|th)\s+seed\b", re.IGNORECASE)
        m = seed_claim_re.search(text)
        if m:
            try:
                claimed_seed = int(m.group(1))
                if abs(claimed_seed - actual_seed) >= 3:
                    problems.append(
                        f"LOTL claims '{claimed_seed}th seed' but standings show {actual_seed}"
                    )
                    critical = True
            except ValueError:
                pass

    # ---- 3. Games-back sanity (NHL uses points, NFL uses games-behind) ----
    league = (TEAMS.get(team_key, {}) or {}).get("espn_league", "").lower()
    if league == "nhl":
        # NHL: "31 games back" is mathematically impossible in most contexts.
        # ESPN labels this field as games-back but it's really a points gap.
        # Flag any claim >= 30 as suspicious.
        gb_m = re.search(r"\b(\d{2,3})\s+games?\s+back\b", text_lower)
        if gb_m:
            try:
                gb = int(gb_m.group(1))
                if gb >= 25:
                    problems.append(
                        f"LOTL says '{gb} games back' for NHL — this is usually points, not games"
                    )
                    critical = True  # This specific Keefe-bug variant is fatal
            except ValueError:
                pass

    # ---- 4. Record contradiction with team_info (defense-in-depth on top of fact_check_lotl) ----
    correct_record = (team_info or {}).get("record") or ""
    if correct_record:
        cparts = correct_record.split("-")
        if len(cparts) >= 2:
            try:
                cw = int(cparts[0])
                cl = int(cparts[1].strip().split()[0])
                # Look for record-like patterns with >30 games played
                for m in re.finditer(r"\b(\d{1,3})[-\u2013\u2014](\d{1,3})\b", text):
                    w, l = int(m.group(1)), int(m.group(2))
                    # Heuristic: season-record if W+L roughly matches & total >= 20
                    if w + l >= 20 and abs((w + l) - (cw + cl)) <= 5:
                        if w != cw or l != cl:
                            problems.append(
                                f"LOTL record {w}-{l} contradicts ESPN record {cw}-{cl}"
                            )
                            critical = True
                            break
    ...
    return (not critical), problems


def _names_match(a, b):
    """Loose name matching: same last word (surname) counts as a match.

    Handles 'Sheldon Keefe' vs 'S. Keefe' vs 'Coach Keefe'.
    """
    if not a or not b:
        return False
    ap = a.strip().split()
...
def fact_check_lotl(text, team_key, team_info, recent, upcoming, phase_info, standings):
    ...
    if ("offseason" in phase_id or "ended" in phase_id or "draft" in phase_id
            or "eliminated" in phase_id or phase_id in ("otas", "training_camp", "combine_free_agency", "pre_draft")):
        game_phrases = ["last night", "tonight's game", "yesterday's game", "beat the", "fell to the", "lost to the",
                        ...
                        "this week's game", "this weekend's game", "kickoff", "tip-off", "faceoff",
                        "final score", "opening drive", "halftime", "closing minutes"]
        text_lower = text.lower()
    ...
    # 5. Hard-Fact verification against league APIs (coach, seed, games-back, record)
    # This is the single most-impactful check: it stops Perplexity hallucinations
    # like "Sheldon Keefe was fired yesterday" or "31 games back" from shipping.
    # We only fetch the coach once per build via a light module-level cache to
    # keep daily runs fast.
    try:
        coach_name = _get_cached_coach(team_key)
    except Exception as e:
        print(f"  HARD-FACT [{team_key}]: coach lookup skipped ({type(e).__name__})")
        coach_name = None
    try:
        ok, hf_problems = verify_hard_facts(text, team_key, team_info, standings, coach_name)
    except Exception as e:
        print(f"  HARD-FACT [{team_key}]: verifier errored ({e}); skipping")
        ok, hf_problems = True, []
    if hf_problems:
        print(f"  HARD-FACT [{team_key}]: {'; '.join(hf_problems)}")
    if not ok:
        print(f"  HARD-FACT [{team_key}]: rejecting LOTL — using deterministic fallback")
        return generate_espn_fallback_lotl(
            team_key, team_info, recent or [], upcoming or [],
            phase_info or {"phase": phase_id or "unknown", "label": ""},
        )
...
# Module-level coach cache keyed by team_key. Avoids re-fetching ESPN coaches
# endpoint on every LOTL pass (once per team per build is plenty).
_COACH_CACHE = {}


def _get_cached_coach(team_key):
    if team_key in _COACH_CACHE:
        return _COACH_CACHE[team_key]
    try:
        name = _fetch_team_coach(team_key)
    except Exception:
        name = None
    _COACH_CACHE[team_key] = name
    if name:
        print(f"  HARD-FACT [{team_key}]: current head coach per ESPN = {name}")
    return name
...
def run_content_diagnostics(db):
    """Cross-field / cross-section content QA pass.

    Character-level validation (Section 0.3, 0.4) catches bad bytes. This
    pass catches editorial errors that slip past sanitize_ascii:

      * cross-section URL duplication (same story in featured AND team.the_latest)
      * week_ahead duplicate games (same day + same teams, e.g. two Jays@Angels on Wed)
      * NHL LOTL using 'games back' (NHL uses points, not games)
      * LOTL seed claim contradicting team 'detail' (e.g. '15th seed' vs '8th in Atlantic')
      * Name-variant inconsistency in LOTL ('Mansoor Delane' vs 'Manor Delane')
      * Streak stat contradicting recent_results W/L pattern
      * ESPN monoculture in article sources

    Destructive corrections (URL dedup, week_ahead dedup) are applied.
    Soft warnings are logged but do not fail the build. Section 0.5 of the
    validator (also being added) is the authoritative gate that will eventually
    enforce these as blocking, once we trust the diagnostics.
    """
    warnings_found = []

    teams = db.get("teams") or {}

    # --- 1. Cross-section URL dedup ---------------------------------------
    # Strategy: if the same article URL appears in a top-section (featured,
    # two_up, extra_story) AND in a team's the_latest, remove from the_latest.
    # If duplicated across two teams' the_latest, keep only the first.
    url_locations = {}

    def _register(url, section_tag):
        if url and isinstance(url, str):
            url_locations.setdefault(url, []).append(section_tag)

    feat = db.get("featured") or {}
    _register(feat.get("link"), "featured")
    for i, s in enumerate(db.get("two_up") or []):
        _register((s or {}).get("link"), f"two_up[{i}]")
    extra = db.get("extra_story") or {}
    _register(extra.get("link"), "extra_story")
    for tk, tdata in teams.items():
        for i, art in enumerate((tdata.get("the_latest") or [])):
            _register((art or {}).get("link"), f"teams.{tk}.the_latest[{i}]")

    removed_from_team_latest = 0
    for url, locs in url_locations.items():
        if len(locs) <= 1:
            continue
        warnings_found.append(f"Duplicate URL across sections ({len(locs)}x): {url[:70]}")
        top_locs = [l for l in locs if not l.startswith("teams.")]
        team_locs = [l for l in locs if l.startswith("teams.")]
        # If it lives in a top section, purge from all team.the_latest copies.
        # Otherwise keep first team.the_latest copy and drop subsequent ones.
        if top_locs:
            drop_targets = team_locs
        else:
            drop_targets = team_locs[1:]
        for loc in drop_targets:
            # loc format: "teams.{tk}.the_latest[{i}]"
            m = re.match(r"teams\.([^.]+)\.the_latest\[(\d+)\]", loc)
            if not m:
                continue
            tk = m.group(1)
            latest = teams.get(tk, {}).get("the_latest") or []
            for art in latest:
                if isinstance(art, dict) and art.get("link") == url and not art.get("_drop"):
                    art["_drop"] = True
                    removed_from_team_latest += 1
                    break
    for tk, tdata in teams.items():
        latest = tdata.get("the_latest") or []
        tdata["the_latest"] = [a for a in latest if isinstance(a, dict) and not a.get("_drop")]
    if removed_from_team_latest:
        LOG.info(f"[diagnostics] Removed {removed_from_team_latest} cross-section duplicate article(s) from team.the_latest")

    # --- 2. week_ahead duplicate game detection ---------------------------
    week = db.get("week_ahead") or {}
    ...
    for g in games:
        if not isinstance(g, dict):
            ...
            continue
        # Key: day + matchup signature (handles both the schedule-row shape
        # {day, team, opp} and the draft-inject shape {day, matchup})
        matchup = g.get("matchup") or f"{g.get('team','')}|{g.get('opp','')}"
        key = (g.get("day", ""), matchup)
        if key in seen_keys:
            warnings_found.append(f"Duplicate week_ahead entry removed: day={key[0]!r} matchup={key[1]!r}")
            dup_count += 1
            continue
        seen_keys.add(key)
        deduped_games.append(g)
    if dup_count:
        week["games"] = deduped_games
        db["week_ahead"] = week
        LOG.info(f"[diagnostics] Removed {dup_count} duplicate week_ahead entr(ies)")

    # --- 3. 'games back' in NHL / NFL LOTL --------------------------------
    # NHL uses points; NFL uses games behind with a specific meaning for
    # divisional standings. Only MLB and NBA commonly use "games back".
    for tk, tdata in teams.items():
        league = tdata.get("league", "")
        body = ((tdata.get("lotl") or {}).get("body") or "")
        if not body:
            continue
        if league == "NHL" and re.search(r"\bgames?\s+back\b", body, re.IGNORECASE):
            warnings_found.append(f"{tk} [NHL] LOTL uses 'games back' -- should be 'points back'")
        if league == "NFL" and re.search(r"\bgames?\s+back\b", body, re.IGNORECASE):
            warnings_found.append(f"{tk} [NFL] LOTL uses 'games back' -- unusual phrasing for NFL")

    # --- 4. LOTL seed contradicting team 'detail' -------------------------
    # e.g. LOTL says '15th seed' while detail says '8th in Atlantic'.
    for tk, tdata in teams.items():
        detail = tdata.get("detail") or ""
        body = ((tdata.get("lotl") or {}).get("body") or "")
        if not detail or not body:
            continue
        detail_seed_m = re.search(r"\b(\d+)(?:st|nd|rd|th)\b", detail)
        if not detail_seed_m:
            continue
        canonical_seed = int(detail_seed_m.group(1))
        # Scan body for "<N>th seed", "<N>th place", "<N>th in the <conference/division>"
        body_seed_pattern = re.compile(
            r"\b(\d+)(?:st|nd|rd|th)\s+(?:seed|place|in\s+the)\b",
            re.IGNORECASE,
        )
        for m in body_seed_pattern.finditer(body):
            claimed = int(m.group(1))
            # Within 2 positions is fine (conference seed vs division rank legitimately differ);
            # 3+ gap is a contradiction.
            if abs(claimed - canonical_seed) >= 3:
                warnings_found.append(
                    f"{tk} LOTL claims '{m.group(0)}' but detail says '{detail}'"
                )

    # --- 5. Name-variant inconsistency in LOTL ----------------------------
    # Catches Perplexity flipping spellings of the same name inside one body,
    # e.g. 'Mansoor Delane' and 'Manor Delane' in the same paragraph.
    for tk, tdata in teams.items():
        body = ((tdata.get("lotl") or {}).get("body") or "")
        if not body:
            continue
        # Strip HTML tags so we don't match e.g. <strong>Sheldon
        plain = re.sub(r"<[^>]+>", "", body)
        # Match 'Firstname Lastname' pairs
        pairs = re.findall(r"\b([A-Z][a-z]{2,})\s+([A-Z][a-z]{2,})\b", plain)
        by_surname = {}
        for first, last in pairs:
            by_surname.setdefault(last, set()).add(first)
        for surname, firsts in by_surname.items():
            if len(firsts) < 2:
                continue
            sorted_firsts = sorted(firsts)
            for i in range(len(sorted_firsts)):
                for j in range(i + 1, len(sorted_firsts)):
                    a, b = sorted_firsts[i], sorted_firsts[j]
                    if a == b:
                        continue
                    if abs(len(a) - len(b)) > 3 or a[0] != b[0]:
                        continue
                    # Rough similarity: how many leading chars agree
                    shared = 0
                    for ca, cb in zip(a, b):
                        if ca == cb:
                            ...
                            break
                    if shared >= min(len(a), len(b)) - 2:
                        warnings_found.append(
                            f"{tk} LOTL name-variant: '{a} {surname}' vs '{b} {surname}'"
                        )

    # --- 6. Streak stat contradicting recent_results ----------------------
    for glance in (db.get("at_a_glance") or []):
        if not isinstance(glance, dict):
            continue
        tk = glance.get("team", "")
        stat = glance.get("stat") or ""
        m = re.match(r"(?:Streak:\s*)?([WL])(\d+)", stat.strip(), re.IGNORECASE)
        if not m:
            continue
        claimed_letter = m.group(1).upper()
        claimed_count = int(m.group(2))
        tdata = teams.get(tk) or {}
        recent = tdata.get("recent_results") or []
        if not recent:
            continue
        actual_letter = None
        actual_count = 0
        for r in recent:
            if isinstance(r, dict):
                letter = (r.get("result") or "")[:1].upper()
            elif isinstance(r, str):
                letter = r[:1].upper()
            else:
                continue
            if letter not in ("W", "L"):
                continue
            if actual_letter is None:
                actual_letter = letter
                actual_count = 1
            elif letter == actual_letter:
                actual_count += 1
            else:
                break
        if actual_letter and (actual_letter != claimed_letter or actual_count != claimed_count):
            warnings_found.append(
                f"{tk} at_a_glance stat '{stat}' but recent_results shows "
                f"{actual_letter}{actual_count}"
            )

    # --- 7. ESPN monoculture -----------------------------------------------
    # Count article sources across all sections. If everything is ESPN,
    # the feed isn't meeting the 'variety of sources' editorial goal.
    source_count = {}
    def _count(art):
        if isinstance(art, dict):
            src = (art.get("source") or "").strip() or "Unknown"
            source_count[src] = source_count.get(src, 0) + 1
    _count(feat)
    for s in (db.get("two_up") or []):
        _count(s)
    _count(extra)
    for tk, tdata in teams.items():
        for art in (tdata.get("the_latest") or []):
            _count(art)
    total_articles = sum(source_count.values())
    if total_articles >= 6 and len(source_count) <= 1:
        only = list(source_count.keys())[0] if source_count else "?"
        warnings_found.append(
            f"Source monoculture: all {total_articles} articles from '{only}'. "
            f"Expected a mix of ESPN / TSN / Sportsnet / NHL.com / MLB.com / Athletic."
        )

    # --- Emit ---------------------------------------------------------------
    if warnings_found:
        LOG.warning(f"[diagnostics] {len(warnings_found)} content warning(s):")
        for w in warnings_found:
            LOG.warning(f"  - {w}")
    else:
        LOG.info("[diagnostics] No content warnings")
...
def build_data():
    ...
    for team_key, cfg in TEAMS.items():
        ...
        team_entry = {
            ...
            # NOTE: live_strip removed 2026-04-22 — was never populated, only
            # ever serialized as null. If a future ticker-integration wants a
            # team-level live strip, re-add it and wire it to a real source.
            "lotl": {
    ...
    # Runs after all sections are assembled. Dedupes URLs and week_ahead games,
    # and surfaces editorial warnings (seed contradictions, name variants,
    # NHL-using-'games-back', source monoculture). See run_content_diagnostics.
